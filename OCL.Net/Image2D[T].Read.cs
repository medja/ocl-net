...
namespace OCL.Net
{
    public sealed partial class Image2D<T>
    {
        ...
        {
            return Read(queue, memory, 0, 0, Width, Height, 0, 0, waitList);
        }
        ...
        public AutoDisposedEvent<TMemory> Read<TMemory>(CommandQueue queue, TMemory memory,
            ...
            where TMemory : MemoryManager<T>
        {
            return Read(queue, memory, (ulong) x, (ulong) y,
            ...
            where TMemory : MemoryManager<T>
        {
            ...
            if (slicePitch > (ulong) memory.Memory.Length * (ulong) sizeof(T))
                throw new ArgumentOutOfRangeException(nameof(memory), "Cannot write over memory capacity");

            var handle = memory.Pin();

            var eventId = ReadInto(queue, (T*) handle.Pointer, x, y, width, height, rowPitch, slicePitch, waitList);
        ...
        public AutoDisposedEvent<Memory<T>> ReadIntoMemory(CommandQueue queue, Memory<T> memory,
            params Event[] waitList)
        {
            return ReadIntoMemory(queue, memory, 0, 0, Width, Height, 0, 0, waitList);
        }
        ...
        public AutoDisposedEvent<Memory<T>> ReadIntoMemory(CommandQueue queue, Memory<T> memory,
            int x, int y, int width, int height, int rowPitch = 0, int slicePitch = 0, params Event[] waitList)
        {
            return ReadIntoMemory(queue, memory, (ulong) x, (ulong) y,
                (ulong) width, (ulong) height, (ulong) rowPitch, (ulong) slicePitch, waitList);
        }
        ...
        {
            return ReadArray(queue, 0, 0, Width, Height, waitList);
        }
        ...
        {
            return ReadArray(queue, (ulong) x, (ulong) y, (ulong) width, (ulong) height, waitList);
        }
            ...
            ulong x, ulong y, ulong width, ulong height, params Event[] waitList)
        {
            ...
            var array = new T[(slicePitch - 1) / (ulong) sizeof(T) + 1];
        ...
        {
            return ReadIntoArray(queue, array, 0, 0, Width, Height, 0, 0, waitList);
        }
        ...
        public AutoDisposedEvent<T[]> ReadIntoArray(CommandQueue queue, T[] array,
            int x, int y, int width, int height, int rowPitch = 0, int slicePitch = 0, params Event[] waitList)
        {
            return ReadIntoArray(queue, array, (ulong) x, (ulong) y,
                (ulong) width, (ulong) height, (ulong) rowPitch, (ulong) slicePitch, waitList);
        }
            ...
            params Event[] waitList)
        {
            ...
            if (slicePitch > (ulong) array.LongLength * (ulong) sizeof(T))
        ...
        {
            return ReadNativeMemory(queue, 0, 0, Width, Height, waitList);
        }
        ...
        public AutoDisposedEvent<NativeMemory<T>> ReadNativeMemory(CommandQueue queue,
            int x, int y, int width, int height, params Event[] waitList)
        {
            return ReadNativeMemory(queue, (ulong) x, (ulong) y, (ulong) width, (ulong) height, waitList);
        }
            ...
            ulong x, ulong y, ulong width, ulong height, params Event[] waitList)
        {
            ...
            var memory = new NativeMemory<T>((long) ((slicePitch - 1) / (ulong) sizeof(T) + 1));
            var handle = memory.Pin();
