...
namespace OCL.Net
{
    public abstract partial class Image1D
    {
        ...
        public static Image1D<T> FromMemory<T>(Context context, Memory<T> memory,
            ...
            where T : unmanaged
        {
            return FromMemory(context, memory, channelOrder, normalized,
                (ulong) width, (ulong) rowPitch, deviceFlags, hostFlags, usePinnedMemory);
        }
            ...
            where T : unmanaged
        {
            return From(context, memory.Span, channelOrder, normalized,
                width, rowPitch, deviceFlags, hostFlags, usePinnedMemory);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static Image1D<T> FromMemory<T>(Context context, Memory<T> memory, ImageFormat format,
            int width = 0, int rowPitch = 0,
            MemoryFlags deviceFlags = DefaultFlags, MemoryFlags hostFlags = DefaultFlags, bool? usePinnedMemory = null)
            where T : unmanaged
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static Image1D<T> From<T>(Context context, Span<T> span,
            ImageChannelOrder channelOrder, bool normalized,
            int width = 0, int rowPitch = 0,
            MemoryFlags deviceFlags = DefaultFlags, MemoryFlags hostFlags = DefaultFlags, bool? usePinnedMemory = null)
            where T : unmanaged
            ...
            where T : unmanaged
        {
            ...
            return From(context, span, format, width, rowPitch, deviceFlags, hostFlags, usePinnedMemory);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static Image1D<T> From<T>(Context context, Span<T> span, ImageFormat format,
            int width = 0, int rowPitch = 0,
            MemoryFlags deviceFlags = DefaultFlags, MemoryFlags hostFlags = DefaultFlags, bool? usePinnedMemory = null)
            where T : unmanaged
        {
        ...
        public static unsafe Image1D<T> From<T>(Context context, Span<T> span, ImageFormat format,
            ulong width = 0, ulong rowPitch = 0,
            MemoryFlags deviceFlags = DefaultFlags, MemoryFlags hostFlags = DefaultFlags, bool? usePinnedMemory = null)
            where T : unmanaged
        {
            ...
            if (rowPitch == 0)
                rowPitch = (ulong) span.Length * (ulong) sizeof(T);

            if (width == 0)
                width = rowPitch / (ulong) format.ElementSize;

            if (rowPitch > (ulong) span.Length * (ulong) sizeof(T))
